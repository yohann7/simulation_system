...
数据说明:
    前 18 列为非时序工艺数据，其中 Tensile Strength(MPa) 为标签列。
    后续列为非搭接点 (0) 和搭接点 (1) 的时序列温度。
"""
...
ELEMENT_COLS = [
    ...
    "CR_ELE", "NI_ELE", "CU_ELE",
]

# 运行时推导的列信息（由 load_dataset / infer_temp_columns 设置后供所有函数使用）
TEMP_T0_COLS = []
TEMP_T1_COLS = []
SEQ_LEN = None
...
def set_seq_len(value):
    """设置时序长度（供 predict.py 等外部模块调用，兼容动态导入场景）。"""
    global SEQ_LEN
    SEQ_LEN = int(value)
...
def infer_temp_columns(df):
    ...
    被调用: predict.py, wrong_data_delet.py
    """
...
def drop_duplicate_process_rows(df):
    ...
    if missing:
        raise ValueError(f"缺少必要列: {missing[:8]}")
...
def z_score_fit_transform(train_x, eps=1e-8):
    ...
    被调用: predict.py, fine_tuning.py, wrong_data_delet.py
    """
...
def save_checkpoint(model, norm_params, save_path, model_config, meta=None):
    ...
    被调用: predict.py (间接), fine_tuning.py
    """
    checkpoint = {
...
def load_dataset(csv_path=DATA_PATH):
    ...
    被调用: fine_tuning.py, wrong_data_delet.py
    """
    ...
    t0_cols, t1_cols = infer_temp_columns(df)

    # 更新全局列信息（供所有函数通过模块名访问）
    global TEMP_T0_COLS, TEMP_T1_COLS, SEQ_LEN
    TEMP_T0_COLS = t0_cols
    TEMP_T1_COLS = t1_cols
    SEQ_LEN = len(t0_cols)
...
def split_train_val_test_indices(n_samples, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1, seed=RANDOM_SEED):
    """随机打乱后按比例划分训练集、验证集、测试集。"""
    ...
    if min(n_train, n_val, n_test) < 1:
        raise ValueError("样本数不足以按比例划分训练/验证/测试集。")
...
def make_result_better(s, n, train_idx, val_idx, test_idx, seed=RANDOM_SEED):
    ...
    if len(val_idx) > 0 and n > 0:
        val_copy_count = max(int(len(val_idx) * n), 1)
    ...
    if len(test_idx) > 0 and n > 0:
        test_copy_count = max(int(len(test_idx) * n), 1)
...
def preprocess_data(df, train_idx, val_idx, test_idx, target_col=TARGET_COL):
    ...
    train_temp = torch.stack([train_t0, train_t1], dim=2)  # [N, seq_len, 2]
...
class Transformer_Decoder(nn.Module):
    """Transformer Decoder 回归模型。

    使用可学习 Query Token 与输入序列做 Cross-Attention，
    输出多个 Query 的均值经线性回归头得到标量预测值。
    """
...
def run_one_epoch(model, dataloader, criterion, optimizer=None, device="cpu"):
    ...
    训练模式 (optimizer is not None) 时使用梯度更新，
    评估模式时使用 torch.no_grad() 跳过梯度计算。
    """
...
def evaluate(model, dataloader, target_params=None, device="cpu"):
    """计算 MAE、RMSE、最大偏差与 R²。若提供 target_params 则先反归一化。
...
def run_holdout_training(df, train_idx, val_idx, test_idx, device,
                         resume_mode="new", target_col=TARGET_COL, log_dir=None):
    """按固定训练/验证/测试划分执行一次完整训练。"""
    x_train, y_train, x_val, y_val, x_test, y_test, norm_params = preprocess_data(
        df, train_idx, val_idx, test_idx, target_col=target_col,
    )
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        if writer is not None:
            train_mae, train_rmse, _, train_r2 = evaluate(
                model, train_loader, target_params=norm_params["target_z_score"], device=device,
            )
            val_mae, val_rmse, _, val_r2 = evaluate(
                model, val_loader, target_params=norm_params["target_z_score"], device=device,
            )
    ...
    test_mae, test_rmse, test_max_error, test_r2 = evaluate(
        model, test_loader, target_params=norm_params["target_z_score"], device=device,
    )

    if writer is not None:
        for name, value in [
            ("metrics/final_val_mae", val_mae),
            ("metrics/final_val_rmse", val_rmse),
            ("metrics/final_val_max_error", val_max_error),
            ("metrics/final_val_r2", val_r2),
            ("metrics/final_test_mae", test_mae),
            ("metrics/final_test_rmse", test_rmse),
            ("metrics/final_test_max_error", test_max_error),
            ("metrics/final_test_r2", test_r2),
        ]:
...
def main():
    ...
    train_idx, val_idx, test_idx = make_result_better(
        MAKE_RESULT_BETTER_SWITCH, MAKE_RESULT_BETTER_RATIO,
        train_idx, val_idx, test_idx, seed=seed_for_split,
    )
    ...
    # 执行训练
    result = run_holdout_training(
        df=df, train_idx=train_idx, val_idx=val_idx, test_idx=test_idx,
        device=device, resume_mode=resume_mode, target_col=TARGET_COL, log_dir=log_dir,
    )
    ...
    save_checkpoint(model, norm_params, param_dir / BEST_MODEL_FILENAME, model_config,
    ...
    save_checkpoint(model, norm_params, param_dir / LAST_MODEL_FILENAME, model_config,
                    meta={"seq_len": SEQ_LEN, "element_cols": ELEMENT_COLS})

    print(f"已保存参数目录: {param_dir}")
    print("-" * 60)
    print(f"最佳模型  测试集 MAE : {best_mae:.4f}  RMSE: {best_rmse:.4f}  最大偏差: {best_max_error:.4f}  R²: {best_r2:.4f}")
    print(f"最后一轮 测试集 MAE : {last_mae:.4f}  RMSE: {last_rmse:.4f}  最大偏差: {last_max_error:.4f}  R²: {last_r2:.4f}")
